...
contract = w3.eth.contract(
    address=w3.toChecksumAddress(config["contract_address"]), abi=config["contract_abi"]
)
...
async def fetch_old_events():
    ...
    while True:
        try:
            ...
            event_filter = contract.events[config["event_name"]].createFilter(fromBlock=start_block, toBlock=to_block)
